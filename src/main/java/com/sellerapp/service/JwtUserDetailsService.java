...
public class JwtUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByOtp(String mobileNumber, String otp) throws UsernameNotFoundException {
		...
		try {
			...
			if (!user.isPresent()) {
				...
				GdmsApiUsers userData = user.get();
				return new User(userData.getMobileNumber(), userData.getPassword(), new ArrayList<>());
			}
	...
	/*
	 * public String getMobileNo(JwtRequest auth) { try { Optional<GdmsApiUsers> us
	 * = gdmsRepository.findByMobileNumber(auth.getMobileNumber()); if
	 * (us.isPresent()) { String mobileNo = us.get().getMobileNumber().substring(0,
	 * 1) + "XXXXXXX" + us.get().getMobileNumber().substring(8, 10); return
	 * mobileNo; } else { // Handle case where user is not found
	 * log.error("User not found for mobile number:  {}" , auth.getMobileNumber());
	 * return null; } } catch (Exception e) { log.error("Exception: {}" ,
