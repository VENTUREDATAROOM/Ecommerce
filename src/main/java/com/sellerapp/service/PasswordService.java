...
public class PasswordService {
	...
	public String changePassword(ChangePasswordDTO changePasswordDTO) {
		try {
			/*
			 * if(!resetPasswordDTO.getNewPassword().equals(resetPasswordDTO.
			 * getConfirmPassword())) { return "pass word did not match"; }
			 */

			/*
			 * Optional<GdmsApiUsers>
			 * us=gdmsRepository.findByMobileNumber(resetPasswordDTO.getMobileNumber());
			 * if(!us.isPresent()) { return "User not found"; }else { GdmsApiUsers
			 * user=us.get(); String
			 * originalPassword=resetPasswordDTO.getOriginalPassword();
			 * if(!bcryptEncoder.matches(originalPassword, user.getPassword())) { return
			 * "old password does not match"; } String
			 * newHashPassword=bcryptEncoder.encode(resetPasswordDTO.getNewPassword());
			 * 
			 * 
			 * user.setPassword(newHashPassword); gdmsRepository.save(user);
			 * System.out.println("Password resent successfully"
			 * +resetPasswordDTO.getMobileNumber());
			 * log.info("password success changed for the user .{}",resetPasswordDTO.
			 * getMobileNumber()); return "Success";
			 * 
			 * }
			 */
